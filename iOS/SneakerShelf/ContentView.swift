...
struct ContentView: View {
    @EnvironmentObject var store: SneakerShelfStore
    @EnvironmentObject var purchases: PurchaseManager
    @State private var showAddSheet = false
    @State private var showSettings = false
    @State private var showPaywall = false
    @State private var editingEntry: SneakerEntry?
    ...
    @ViewBuilder
    private func row(for entry: SneakerEntry) -> some View {
        HStack {
...
struct EntryEditorView: View {
    ...
    let existing: SneakerEntry?
    let onSave: (SneakerEntry) -> Void

    init(entry: SneakerEntry?, onSave: @escaping (SneakerEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            Form {
                Section("Sneaker") {
                    ...
                    TextField("Colorway / Size", text: $detail)
            ...
            .navigationTitle(existing == nil ? "Add Sneaker" : "Edit Sneaker")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? SneakerEntry(name: name, detail: detail, date: date)
                        entry.name = name
                        entry.detail = detail
                        entry.date = date
